...
the API credentials that will be used by the Lambda function for DEV, SIT, UAT, and PROD environments.
 Considering that the Lambda function is storing sensitive database and API credentials,
  ...
  from seeing these credentials in plain text? Select the best option that provides maximum security.`,
    answers: {
      a: `There is no need to do anything because, by default,
      ...
      c: `AWS Lambda does not provide encryption for the environment variables. 
        Deploy your code to an EC2 instance instead `,
      d: `Create new KMS keys and use it to enable encryption helpers that that 
  ...
  },
  {
    question: `
       ...
       Which of the following would satisfy this requirement?
        `,
    answers: {
      a: `Create a new Amazon SNS FIFO topic for the other team. 
      ...
      c: `Set up another Amazon SQS queue for the other team. 
        Grant Amazon S3 permission to send a notification to the second SQS queue`,
      d: `Set up an Amazon SNS topic and configure two Amazon SQS queues to poll the SNS topic.
...
As the Solutions Architect, which of the following should you do to meet the above requirement?`,
    answers: {
      ...
      c: `Set up an IAM Policy and MFA which requires the Cloud Engineers to 
        enter their IAM credentials and token before they can access the ElastiCache cluster`,
      d: `Authenticate the users using Redis AUTH by creating a new Redis Cluster
...
Which of the following is the most suitable solution that you should implement to satisfy this requirement?
        `,
    answers: {
      a: `Configure your CloudFront distribution to use Match Viewer 
      ...
      c: `Configure your CloudFront distribution to use Field-Level Encryption to 
...
Which of the following is the most suitable solution to meet the requirement? 
        `,
    answers: {
      a: `Convert the database schema using the AWS Schema Conversion Tool and AWS Database Migration Service.
...
Which of the following approach can be used to fulfill this requirement? (Select TWO.)
        `,
    answers: {
      a: `Use Amazon S3 and create a lifecycle policy that will move the objects to Amazon S3 Glacier after 2 years.
...
as well as your Aurora database cluster to achieve this requirement?
        `,
    answers: {
      a: `Configuring your application to use the reader endpoint for both production traffic
      ...
      b: `In your application, use the instance endpoint of your Aurora database to handle the 
        incoming production traffic and use the cluster endpoint to handle reporting queries`,
      c: `Do nothing since by default, Aurora will automatically direct the production
         traffic to your high-capacity instances and the reporting queries to your low-capacity instances`,
      d: `creating a custom endpoint in Aurora based on the specified criteria for 
...
https://tutorialsdojo.com/amazon-aurora/
 ...
 As the Solutions Architect, which combination of options should you implement in this scenario? (Select TWO.)
    ...
    answers: {
      ...
      b: `Use the AWS Identity and Access Management service to set up cross-account access 
...
Which of the following should the Architect implement to satisfy this requirement?
    ...
    answers: {
      ...
      c: "vvvvvvvvvvvvvvvffffff",
      d: `Set up and configure Amazon Inspector to send out alert notifications 
...
`,
    answers: {
      ...
      c: `Creating an IAM Role and assigning it to your EC2 instances which 
...
Which of the following is the best method to quickly and temporarily deny access from the specified IP addresses?
 `,
    answers: {
      ...
      c:
        "Adding a rule in the Security Group of the EC2 instances to deny access from the IP Address block",
...
//   const result=  arrayOfObject.map(item=>{
//     return {[item.question]:[item.answers.a,item.answers.b,item.answers.c, item.answers.d]}
// })
// return result
// }

//get all questios that contains the search terms
function transformQuestionArray(
  arrayOfObject,
  questionNumber,
  searchTerm = ""
) {
  ...
  const searchResultArray = arrayOfObject.filter((searchItem) => {
    const questionToBeChecked = searchItem.question;
    //can also use includes, e.g mainString.includes("subString")
    if (
      questionToBeChecked.toLowerCase().indexOf(searchTerm.toLowerCase()) > -1
    ) {
      return searchItem;
    }
  });
  ...
  if (searchResultArray.length > 0) {
    result = searchResultArray.map((item) => {
      if (item.answers.e === undefined) {
        return {
          [item.question]: [
            item.answers.a,
            item.answers.b,
    ...
    alert("no question available for this search term");
    return;
  }
  //shuffle the options
  randomizeArray(result);

  //ensure that the number of question choosen is not greater than available question
  if (questionNumber <= result.length) {
    let arraySlice = result.slice(0, questionNumber);
    return arraySlice;
  } else {
    alert(
      `choosen question number more than available question resources,only ${result.length} questions available for this search term`
    );
    // return [{"Hello Please choose a lesser number":["--------","-------","------","-------"]}]

    return false;
  }
}

export { solArcJonBon1, transformQuestionArray };
